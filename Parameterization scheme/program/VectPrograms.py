...
from phytokens.VectTokens import VectTokens
from utils import execute as Exec
from utils import free_const
from utils import dimensional_analysis as phy
from program.Program import Program
from program.Cursor import Cursor
...
try :
    ...
    warnings.warn("can't import packages")
...
class VectPrograms:
    ...
    max_time_step : int
        Max number of phytokens programs can contain.
    shape : (int, int)
        Shape of batch (batch_size, max_time_step,).

    library : library.Library
        Library of phytokens that can appear in programs.
    n_choices : int
        Number of choosable phytokens.
    n_library : int
        Number of phytokens in the library (including placeholders).
    superparent_idx : int
    ...
    lib_names : numpy.array of shape (n_library,) of str
        Names of phytokens in the library.
    lib_vect : phytokens.Tokens of shape (1, n_library,)
        Vectorized phytokens of library.

    curr_step : int
        Current time step (ie. index of last phytokens added).
    safe_max_time_step : int
        Number of phytokens that can safely be contained in programs if there are chosen completely randomly. (ie. without
    ...
    units_analysis_cases : numpy.array of shape (batch_size, max_time_step,) of int
        Dimensional analysis assignment case code. Units requirement was performed on phytokens if > -1.

    tokens : phytokens.Tokens of shape (batch_size, max_time_step,)
        Vectorized phytokens of contained in batch (including idx in library ie. nature of phytokens).
    ...
    def __init__(self, batch_size, max_time_step, library, candidate_wrapper=None):
        ...
        # (only keeping jit-able vectors in Tokens)
        self.lib_vect = library.properties  # Tokens
        ...
        # Number of phytokens that can be appended safely ie. even
        # in the worst case scenario which generates the maximum number
        # of dummies  where only phytokens of arity = Tok.MAX_ARITY are
        ...
        self.safe_max_time_step = np.floor(  # int
            (self.max_time_step - 1) / config.positional_token_default_values['MAX_NB_CHILDREN']).astype(int)

        # ---------------------------- PROGRAM MANAGEMENT ---------------------------- -> batch dim
        # Individual lengths of programs <= max_time_step,
        # phytokens after n_lengths are dummies or do not have meaning
        self.n_lengths = np.zeros(self.batch_size, dtype=int)  # (batch_size,) of int
        # Number of phytokens needed to finish the program
        ...
        self.tokens = VectTokens(shape=self.shape, invalid_token_idx=self.invalid_idx)  # (batch_size, max_time_step,)
        ...
        self.units_analysis_cases = np.full(shape=self.shape, fill_value=phy.UNITS_ANALYSIS_NOT_PERFORMED_CASE_CODE,
    ...
    def lib(self, attr):
        """
        Gives access to vectorized properties of phytokens in library without having to use [0, :] (as batch_size = 1 in
    ...
    def append(self, new_tokens_idx, forbid_inconsistent_units=False):
        """
        Appends new phytokens to batch.
        New phytokens appended to already complete programs (ie. out of tree phytokens) are ignored.
        ...
        new_tokens_idx : numpy.array of shape (batch_size,) of int
            Index of phytokens to append in the library.要附加到库中的令牌的索引。
        forbid_inconsistent_units : bool
            If True, forbids (by raising an error) appending of new phytokens having constraining units inconsistent
            with current units constraints. Consistency of new phytokens' physical units vs the current programs is
        ...
        assert (new_tokens_idx.dtype == int or new_tokens_idx.dtype == np.dtype(
            "int64")), "Arg new_tokens_idx must be a numpy array of dtype = int"
        ...
        # ------------------ Assert enough space for new phytokens' dummies ------------------
        # Raise error if number of dummies needed to handle new phytokens exceeds max_time_step

        # Space necessary in time dim to finish the program if new phytokens were appended
        n_space_necessary = self.lib("arity")[new_tokens_idx] + self.n_dummies + self.n_lengths  # (batch_size,) of int

        # Is it impossible to append new phytokens to program : mask
        mask_impossible_to_append = n_space_necessary > self.max_time_step  # (batch_size,) of bool

        if mask_impossible_to_append.any():
            error_msg = "Can not append phytokens :\n%s\n which have arities :\n%s\n to programs :\n%s\n as the number " \
                        "of phytokens that would be required to finish the program would then be of " \
                        "(n_dummies + new phytokens arity) =\n%s\n which would exceed max_time_step = %i" \
        ...
        # --------------------------------------------------------------------------------------------------------------
        # Completed programs do not need new phytokens, replacing them by void invalid phytokens.

        # Number of complete programs
        self.n_complete = self.is_complete.sum()  # int
        # Replacing new phytokens trying to be added to complete programs by void
        ...
        # Are new phytokens units equal to legacy units ie dummy units they are replacing: mask
        mask_units_unequal = np.logical_not(
            (units_from_dummies == units_from_new_tokens).all(axis=1))  # (batch_size,) of bool

        # Are new phytokens inconsistent with current units constraints: mask
        # (ie. new phytokens has different units AND has constraints AND legacy dummy has constraints) (avoids NAN != NAN)
        ...
        # mask: was units analysis performed for this step if it was necessary to decide if new phytokens' units are
        ...
        # Program is physical if it was already considered as physical AND new phytokens have consistent units AND units
        # requirement was computed for this step (otherwise phytokens could seem to have consistent units just because
        # units requirements were not performed thus making any phytokens look acceptable).
        ...
        if mask_inconsistency.any() and forbid_inconsistent_units:
            error_msg = "Can not append new phytokens as %i of them (phytokens: %s) have inconsistent units regarding " \
                        "current units constraints.\nThese phytokens respectively have units:\n %s \nBut according to " \
        ...
        # --------------------------------------------------------------------------------------------------------------
        # Replacing dummy by new phytokens (but keeping positional + units info)

        # Is the phytokens a new phytokens: mask
        ...
        # --- UNITS INFO ---
        # Number of units constraining new phytokens
        n_constraining_new_tokens = mask_is_constraining_new_tokens.sum()  # int
        # Adding unit info for new phytokens which have units constraints (those that don't will inherit
        ...
        # to new phytokens' units.
        self.set_units(
            coords_dest=self.coords_of_step(step=self.curr_step)[:, mask_is_constraining_new_tokens],
            # (2, n_constraining_new_tokens,) of int
            new_is_constraining_phy_units=np.full(n_constraining_new_tokens, True),
            # (n_constraining_new_tokens,) of bool
            new_phy_units=units_from_new_tokens[mask_is_constraining_new_tokens],
            # (n_constraining_new_tokens, Tok.UNITS_VECTOR_SIZE) of float
        )
        ...
        # Number of dummies that will be left after new phytokens replace
        ...
        # Update of arities over time dim with new phytokens
        ...
        self.is_complete[
            np.logical_and(self.is_complete == False, self.n_dummies == 0)] = True  # (bool_array.sum(),) of int

        # Number of dummies necessary to complete program now that new phytokens were added
        ...
        # Shifting legacy dummies (except the one replaced by phytokens) to make room for new dummies

        # -------- BEFORE SHIFT COORDS --------
        # Is phytokens a legacy phytokens (before shift): mask
        mask_legacy_tokens_noshift = np.logical_and.reduce((  # (batch_size, max_time_step,) of bool
            ...
            self.tokens.pos < self.tile_batch_vect(n_legacy_dummies) + self.curr_step,
        # (batch_size, max_time_step,) of bool
        ))
        # Coords of legacy phytokens
        n_legacy_dummies_total, coords_legacy_tokens_noshift = self.mask_to_coords(
            mask_legacy_tokens_noshift)  # int, (2, n_legacy_dummies_total) of int

        # -------- SHIFTED COORDS --------
        # Amount by which legacy phytokens should be shifted in time dim (array over batch dim)
        # Ie. amount of space to leave for new dummies that will be added between new phytokens and legacy dummies
        legacy_shift = n_new_dummies[  # (n_legacy_dummies_total,) of int
            coords_legacy_tokens_noshift[0]
            # coords of legacy phytokens in batch dim      # (n_legacy_dummies_total,) of int
        ]
        # Legacy phytokens shift
        ...
        # Mask corresponding to legacy phytokens after shift
        mask_legacy_tokens_shifted = self.coords_to_mask(
            coords_legacy_tokens_shifted)  # (batch_size, max_time_step,) of bool

        # -------- PERFORMING SHIFT --------
        # Performing shift if necessary
        # (ie. if there are legacy phytokens)
        ...
        # Is phytokens coord an emplacement for a new dummy: mask
        # ie. where pos >= step AND pos < (n_new_dummies + step)
        mask_new_dummies = np.logical_and.reduce((  # (batch_size, max_time_step,) of bool
            self.tokens.pos >= self.curr_step,  # (batch_size, max_time_step,) of bool
            self.tokens.pos < self.tile_batch_vect(n_new_dummies) + self.curr_step,
        # (batch_size, max_time_step,) of bool
        ))

        # Coords of new dummies to create
        n_new_dummies_total, coords_new_dummies = self.mask_to_coords(
            mask_new_dummies)  # int, (2, n_new_dummies_total) of int
        ...
        self.set_children(coords_dest=coords_new_dummies, has_mask=False,
                          pos_val=config.positional_token_default_values['INVALID_POS'], nb=0, )
        ...
        n_new_dummies_wo_sibling, coords_new_dummies_wo_sibling = self.mask_to_coords(
            # int, (2, n_new_dummies_wo_sibling) of int
            mask=mask_new_dummies_wo_siblings)
        self.set_siblings(coords_dest=coords_new_dummies_wo_sibling, has_mask=False,
                          pos_val=config.positional_token_default_values['INVALID_POS'], nb=0)
        ...
        self.tokens.has_siblings_mask[
            tuple(coords_new_dummies_w_siblings_0)] = True  # (n_new_dummies_w_sibling_0,) of bool
        self.tokens.has_siblings_mask[
            tuple(coords_new_dummies_w_siblings_1)] = True  # (n_new_dummies_w_sibling_1,) of bool
        # Setting sibling relationships : pos
        self.tokens.siblings_pos[tuple(coords_new_dummies_w_siblings_0)] = \
            self.tokens.pos[tuple(coords_new_dummies_w_siblings_1)][:,
            np.newaxis]  # sibling 0's sibling is sibling 1 # (n_new_dummies_w_sibling_0,) of int
        self.tokens.siblings_pos[tuple(coords_new_dummies_w_siblings_1)] = \
            self.tokens.pos[tuple(coords_new_dummies_w_siblings_0)][:,
            np.newaxis]  # sibling 1's sibling is sibling 0 # (n_new_dummies_w_sibling_1,) of int
        ...
        # with time dim = previous step (=step of new phytokens)
        coords_new_dummies_parents = np.stack((  # (2, n_new_dummies_total,) of int
            ...
            # = self.phytokens.pos[coords_new_dummies[0], self.curr_step - 1]
        ), axis=0)
        # Computing depth
        ...
        self.tokens.ancestors_pos[tuple(coords_new_dummies)] = self.tokens.ancestors_pos[
            tuple(coords_new_dummies_parents)]
        self.register_ancestor(coords_new_dummies)

        # --------------------------------------------------------------------------------------------------------------
        # --------------------------------- COMPLETING WITH DUMMIES : INFORMING PARENT ---------------------------------
        # --------------------------------------------------------------------------------------------------------------
        # Informing parent of new dummies (ie. new phytokens) that they have a child

        # Is phytokens part of an incomplete program : mask
        mask_incomplete = np.logical_not(self.tile_batch_vect(self.is_complete))

        # -------- HANDLING LONELY CHILDREN --------
        # New phytokens having only one dummy child : mask
        # Ie. is a new phytokens AND has arity == 1 AND program is not complete
        ...
        pos_lonely_children = np.stack((  # (n_new_tokens_w_lonely_child, 1,) of int
            ...
            np.full(n_new_tokens_w_lonely_child, config.positional_token_default_values['INVALID_POS'], int)),
            # no 2nd child
            axis=1, )
        # Setting children
        self.set_children(coords_dest=tuple(coords_new_tokens_w_lonely_child),
        ...
        # New phytokens having two children : mask
        # Ie. is a new phytokens AND has arity == 2 AND program is not complete
        ...
        pos_double_children = np.stack((  # (n_new_tokens_w_two_children, 2,) of int
            ...
            axis=1)
        # Setting children
        self.set_children(coords_dest=tuple(coords_new_tokens_w_two_children),
        ...
        # requirement of dummy representing next phytokens to guess.
    ...
    def set_programs(self, tokens_idx, forbid_inconsistent_units=False):
        ...
        tokens_idx : numpy.array of shape (batch_size, int <= max_time_step) of int
            Index of phytokens making up programs in the library.
    ...
    def assign_required_units(self, step=None, ignore_unphysical=True, ):
        ...
        ----------
        step : int or None
            Required units assignment routine is run on phytokens at step. By default, step = current step.
        ...
        # Assertion
        assert step < self.max_time_step, "Can not work on phytokens at step = %i as this step is out of range " \
    ...
    def get_parent(self, coords):
        """
        Get parent's coordinates of phytokens at coords.
        Parameters
        ----------
        coords : numpy.array of shape (2, ?) of int
            Coords of phytokens, 0th array in batch dim and 1th array in time dim.
        Returns
        -------
    ...
    def get_siblings(self, coords):
        """
        Get siblings' coordinates of phytokens at coords.
        Parameters
        ----------
        coords : numpy.array of shape (2, ?) of int
            Coords of phytokens, 0th array in batch dim and 1th array in time dim.
    ...
    def get_children(self, coords):
        """
        Get children's coordinates of phytokens at coords.
        Parameters
        ----------
        coords : numpy.array of shape (2, ?) of int
            Coords of phytokens, 0th array in batch dim and 1th array in time dim.
    ...
    def get_ancestors(self, coords):
        """
        Get ancestors' coordinates of phytokens at coords.
        Parameters
        ----------
        coords : numpy.array of shape (2, ?) of int
            Coords of phytokens, 0th array in batch dim and 1th array in time dim.
    ...
    def get_parent_idx(self, coords, no_parent_idx_filler=None):
        """
        Get parents idx of phytokens at coords.
        Parameters
        ----------
        coords : numpy.array of shape (2, ?,) of int
            Coords of phytokens, 0th array in batch dim and 1th array in time dim.
        no_parent_idx_filler : int
            Fill value to return where phytokens have no parent.
        Returns
        -------
        ...
        # What to put as placeholder where phytokens at coords do not have a target relationship
        if no_parent_idx_filler is None:
            no_target = self.library.invalid_idx
        else:
            no_target = no_parent_idx_filler

        # coords of target related to phytokens at coords
        coords_target = self.get_parent(coords)
        # mask : do phytokens at coords have a target relationship
        have_target = self.tokens.has_parent_mask[tuple(coords)]

        # initializing result
        target_idx = np.full(coords.shape[1], no_target, dtype=int)  # (?,) of int
        # handling phytokens at coords having a target relationship
        ...
        # handling phytokens at coords NOT having a target relationship
    ...
    def get_sibling_idx(self, coords, no_sibling_idx_filler=None):
        """
        Get siblings idx of phytokens at coords.
        Parameters
        ----------
        coords : numpy.array of shape (2, ?,) of int
            Coords of phytokens, 0th array in batch dim and 1th array in time dim.
        no_sibling_idx_filler : int
            Fill value to return where phytokens have no sibling.
        Returns
        -------
    ...
    def get_ancestors_idx(self, coords, no_ancestor_idx_filler=None):
        """
        Get ancestors idx of phytokens at step.
        Parameters
        ----------
        coords : numpy.array of shape (2, ?,) of int
            Coords of phytokens, 0th array in batch dim and 1th array in time dim.
        no_ancestor_idx_filler : int
            Fill value to return where phytokens have no ancestors and for padding family tree lines.
        Returns
        -------
        ancestors_idx : numpy.array of shape (?, max_n_ancestors = max_time_step) of int
            Idx of all ancestors for each phytokens of batch. For phytokens having has_ancestors_mask = False, a vector filled
            with no_ancestor_idx_filler is given. Family lines of size < max_time_step contain invalid phytokens which is
        ...
        # Number of phytokens to handle here: n_tokens = ?
        n_tokens = coords.shape[1]

        # --- COORDS ---
        # coords of target related to phytokens at coords
        coords_target = self.get_ancestors(coords)  # (max_n_relatives+1, ?)
        # Number of targets for each phytokens (shape[0] - 1 because Oth dim stores batch_dim coords and others store time
        ...
        # mask : is target valid ie. it is related to a phytokens having a target relationship (n_tokens dim) AND it is not
        # just a padding value but a real target (relatives dim)
        mask_valid_target = np.tile(self.tokens.n_ancestors[tuple(coords)],
                                    reps=(max_n_relatives, 1)).transpose() > relatives_tile  # (?, max_n_relatives)
        ...
        coords_target_slicing[:, :, 0] = np.tile(coords_target[0, :],
                                                 reps=(max_n_relatives, 1)).transpose()  # (?, max_n_relatives)
        ...
        coords_target_slicing[:, :, 1][mask_valid_target] = coords_target[1:, :].transpose()[
            mask_valid_target]  # (?, max_n_relatives)
    ...
    def get_parent_idx_of_step(self, step=None, no_parent_idx_filler=None):
        """
        Get parents idx of phytokens at step.
        Parameters
        ----------
        step : int
            Step of phytokens from which parent idx should be returned.
        ...
        if step is None:
            step = self.curr_step
        # Assertion
        assert step < self.max_time_step, "Can not get information on phytokens at step = %i as this step is out of range " \
                                          "(max_time_step = %i)" % (step, self.max_time_step)
    ...
    def get_sibling_idx_of_step(self, step=None, no_sibling_idx_filler=None):
        """
        Get siblings idx of phytokens at step.
        Parameters
        ----------
        step : int
            Step of phytokens from which sibling idx should be returned.
            By default, step = current step
        no_sibling_idx_filler : int
            Fill value to return where phytokens have no sibling.
        Returns
    ...
    def get_ancestors_idx_of_step(self, step=None, no_ancestor_idx_filler=None):
        ...
        ----------
        step : int
            Step of phytokens from which ancestors idx should be returned.
            By default, step = current step
        no_ancestor_idx_filler : int
            Fill value to return where phytokens have no ancestors and for padding family tree lines.
        Returns
    ...
    def count_tokens_idx(self, tokens_idx):
        """
        Creates a library size vector containing count of phytokens idx (along ?1 dim) in tokens_idx for each line of
        tokens_idx. Eg: [ [2, 2, 1, 2, 5], [1, 1, 1, 0, 4] ] -> [0,1,3,0,0,1] [1,3,0,0,1,0] assuming n_library = 5.
        Parameters
        ----------
        tokens_idx : numpy.array of shape (?, ?1) of int
            Sample of phytokens vectors.
        Returns
        -------
    ...
    def get_property_of_relative(self, coords, relative, attr):
        """
        Returns the attribute (eg. phy_units, arity etc.) of the [relative] of phytokens at coords. Fills with default
        value of attribute in Tokens where phytokens at coords do not have [relative].
        Parameters
        ----------
        coords : numpy.array of shape (2, ?,) of int
            Coords of phytokens, 0th array in batch dim and 1th array in time dim.
        ...
        assert relative in supported_relationships, "%s not supported, use one of %s" % (
        relative, supported_relationships)

        # Number of phytokens considered here = ?
        n_tokens = coords.shape[1]
        ...
        # Number of phytokens having relatives that do exist
        ...
        attribute = np.full(shape=attribute_shape, fill_value=attribute_filler_value,
                            dtype=attribute_type)  # (?, (+ depending on attr))                                                                                    # (?,)
        ...
        attribute[is_meaningful] = self.tokens.__getattribute__("%s" % attr)[
            tuple(coords_existing_relatives)]  # (n_meaningful,)
    ...
    def set_parent(self, coords_dest, has_mask, pos_val):
        """
        Sets parent properties of phytokens of coordinates coords_dest with new values given in args.
        Parameters
        ----------
    ...
    def set_children(self, coords_dest, has_mask, pos_val, nb):
        """
        Sets children properties of phytokens of coordinates coords_dest with new values given in args.
        Parameters
        ----------
        coords_dest : numpy.array of shape (2, ?) of int
            Coords where to set property, 0th array in batch dim and 1th array in time dim.
    ...
    def set_siblings(self, coords_dest, has_mask, pos_val, nb):
        """
        Sets siblings properties of phytokens of coordinates coords_dest with new values given in args.
        Parameters
        ----------
        coords_dest : numpy.array of shape (2, ?) of int
            Coords where to set property, 0th array in batch dim and 1th array in time dim.
    ...
    def set_ancestors(self, coords_dest, has_mask, pos_val, nb):
        """
        Sets ancestors properties of phytokens of coordinates coords_dest with new values given in args.
        Parameters
        ----------
        coords_dest : numpy.array of shape (2, ?) of int
            Coords where to set property, 0th array in batch dim and 1th array in time dim.
        ...
        pos_val : numpy.array of shape (?, max_time_step) of int
            New value to set.
        nb : numpy.array of shape (?,) of int
            Number of ancestors counting the phytokens itself as its own ancestor.
        """
    ...
    def register_ancestor(self, coords_dest, ):
        """
        Registers phytokens located at coords_dest in their own ancestor records (as a phytokens counts as its own ancestor)
        and updates the number of ancestors. Depths must be up-to-date for this function to perform correctly.
        Parameters
        ----------
        coords_dest : numpy.array of shape (2, ?) of int
            Coords of phytokens which's ancestors records should be updated, 0th array in batch dim and 1th array in time dim.
        """
        # ? = number of phytokens which need their ancestor to be updated
        n_tokens = coords_dest.shape[1]

        # Records of ancestors positions for phytokens at coords_dest (ie vectors of size ? of family lines)
        ...
        # registering phytokens as their own ancestors, this is performed at their own depth in the family line.
        coords_new_ancestors = np.stack((  # (2, ?,) of int
            np.arange(n_tokens),
            # phytokens dim coord (always = [1,2,3..] because records_ancestors_pos is already the subset of interest)
            self.tokens.depth[tuple(coords_dest)],  # ancestor line dim coord ie own depth of phytokens to affect
        ), axis=0)

        # Registering phytokens as their own ancestors ie. adding own phytokens positions (time dim) in their records
    ...
    def set_units(self, coords_dest, new_is_constraining_phy_units, new_phy_units):
        """
        Sets units properties of phytokens of coordinates coords_dest with new values given in args.
        Parameters
        ----------
        coords_dest : numpy.array of shape (2, ?) of int
            Coords where to set new phytokens, 0th array in batch dim and 1th array in time dim.
    ...
    def set_static_units_from_idx(self, coords_dest, tokens_idx_src):
        """
        Sets units properties of phytokens at coords_dest from units of phytokens tokens_idx_src as given in the library.
        Parameters
        ----------
        coords_dest : numpy.array of shape (2, ?) of int
            Coords where to set new phytokens, 0th array in batch dim and 1th array in time dim.
        tokens_idx_src : numpy.array of shape (batch_size,)
            Index of phytokens from which units should be taken.
        """
    ...
    def set_non_positional_from_idx(self, coords_dest, tokens_idx_src):
        """
        Sets non_positional properties and index of new phytokens as given in the library.
        Parameters
        ----------
        coords_dest : numpy.array of shape (2, ?) of int
            Coords where to set new phytokens, 0th array in batch dim and 1th array in time dim.
        tokens_idx_src : numpy.array of shape (batch_size,)
            Index of new phytokens.
        """
        # ------------------------ non_positional ------------------------
        # Index
    ...
    def move_dummies(self, coords_src, coords_dest, do_update_relationships_pos=True, do_fill_with_void=True):
        ...
        ! This function only works for dummies placed as placeholders that are completing programs. !
             -> Does not work for moved phytokens that have sibling relationships to each others
        ...
        coords_src : numpy.array of shape (2, ?) of int
            Coords of phytokens to move, 0th array in batch dim and 1th array in time dim.
        coords_dest : numpy.array of shape (2, ?) of int
            Coords where to move phytokens, 0th array in batch dim and 1th array in time dim.
        do_update_relationships_pos : bool
            If True, updates position of moved phytokens in other phytokens representing family relationships
            (parent, siblings,).
            ! Does not update siblings relationships between moved phytokens and does not update children. !
        do_fill_with_void : bool
            If True, fills coords_src with void of invalid phytokens after move where there is no overlap
        ...
        self.tokens.is_constraining_phy_units[tuple(coords_dest)] = self.tokens.is_constraining_phy_units[
            tuple(coords_src)]
        self.tokens.phy_units[tuple(coords_dest)] = self.tokens.phy_units[tuple(coords_src)]

        # ------------ Positional properties ------------
        # Must not be copied
        # self.phytokens.pos                       [tuple(coords_dest)] = self.phytokens.pos                       [tuple(coords_src)]
        # self.phytokens.pos_batch                 [tuple(coords_dest)] = self.phytokens.pos_batch                 [tuple(coords_src)]
        # ---- Depth ----
        ...
        def update_relationships_pos_of_moved_tokens(coords_b_move, coords_a_move):
            """
            Updates position of dummy phytokens that were moved in other phytokens corresponding to family relationships
            (parent, siblings).
            ! This function only works for dummies placed as placeholders that are completing programs. !
                 -> Does not work for moved phytokens that have sibling relationships to each others
                    and does not update dummies' children nor their ancestors. This function works for
            ...
            coords_b_move : numpy.array of shape (2, ?) of int
                Coords of phytokens before move, 0th array in batch dim and 1th array in time dim.
            coords_a_move : numpy.array of shape (2, ?) of int
                Coords of phytokens after move, 0th array in batch dim and 1th array in time dim.
            """

            # ------------ Updating ancestors pos ------------
            # (as the dummy phytokens counts as its own ancestor, and it was just moved)
            self.register_ancestor(coords_dest=coords_a_move)

            # ------------ Informing parent ------------

            # Do moved phytokens have parents : mask
            ...
            coords_valid_parent = np.stack((  # (2, ?0,) of int
                ...
                coords_a_move[0],  # batch dim coord -> same as phytokens     # (?,) of int
            ...
            # pos of children of parent of moved phytokens
            pos_parent_children = self.tokens.children_pos[tuple(coords_valid_parent)]  # (?0, Tok.MAX_NB_CHILDREN)

            # Pos (before move) of moved phytokens which have parents, tiled to match Tok.MAX_NB_CHILDREN
            # must use coords_b_move
            pos_tokens_w_parent_b_move_tiled = np.tile(  # (2, ?0,) of int
                coords_b_move[1],  # 'batch' of pos (before move) of moved phytokens (in time dim)         # (?,) of int
                (config.positional_token_default_values['MAX_NB_CHILDREN'], 1,)
            ).transpose()[mask_has_parent, :]

            # Pos (after move) of moved phytokens which have parents
            pos_tokens_w_parent_a_move = coords_a_move[1][mask_has_parent]  # (?0,) of int

            # Update the right pos child (ie. the one where child pos = pos of phytokens before move)
            ...
            # Do moved phytokens have siblings : mask
            ...
            pos_valid_sibling = self.tokens.siblings_pos[tuple(coords_a_move)][mask_has_sibling,
                                :]  # (?1, Tok.MAX_NB_SIBLINGS) of int

            # Coords of siblings when available
            coords_valid_sibling = np.stack((  # (2, ?1,) of int
                coords_a_move[0][mask_has_sibling],  # batch dim coord -> same as phytokens     # (?1,) of int
                pos_valid_sibling[:, 0],  # time dim coord  -> siblings           # (?1,) of int
            ), axis=0
            )

            # Position of sibling's sibling
            pos_sibling_sibling = self.tokens.siblings_pos[
                tuple(coords_valid_sibling)]  # (?1, Tok.MAX_NB_SIBLINGS) of int

            # Updating sibling's sibling pos to phytokens pos (after move)
            pos_sibling_sibling = coords_a_move[1][mask_has_sibling][:, np.newaxis]  # (?1, Tok.MAX_NB_SIBLINGS) of int

            # Update matrix of properties
            self.tokens.siblings_pos[
                tuple(coords_valid_sibling)] = pos_sibling_sibling  # (?1, Tok.MAX_NB_SIBLINGS) of int
    ...
    def fill_with_void(self, coords_dest):
        """
        Helper function that fills coords_dest with void invalid phytokens.
        Parameters
        ----------
        coords_dest : numpy.array of shape (2, ?) of int
        ...
        # self.phytokens.pos
        # self.phytokens.pos_batch
        # ---- Depth ----
        self.tokens.depth[tuple(coords_dest)] = config.positional_token_default_values[
            'INVALID_DEPTH']  # (?,) of int
        # ---- Family relationships ----
        # Token family relationships: family mask
        ...
        # Token family relationships: pos
        self.tokens.parent_pos[tuple(coords_dest)] = config.positional_token_default_values[
            'INVALID_POS']  # (?,) of int
        self.tokens.siblings_pos[tuple(coords_dest)] = config.positional_token_default_values[
            'INVALID_POS']  # (?,) of int
        self.tokens.children_pos[tuple(coords_dest)] = config.positional_token_default_values[
            'INVALID_POS']  # (?,) of int
        self.tokens.ancestors_pos[tuple(coords_dest)] = config.positional_token_default_values[
            'INVALID_POS']  # (?,) of int
        # Token family relationships: numbers
    ...
    def compute_sum_arities(self, step=0):
        ...
        # Arities of real phytokens only
    ...
    def get_token(self, coords):
        """
        Returns phytokens objects at coords.
        Parameters
        ----------
        coords : numpy.array of shape (2, ?) of int
            Coords of phytokens to return, 0th array in batch dim and 1th array in time dim.
        Returns
        -------
        phytokens : numpy.array of shape (?,) of phytokens.Token
        """
        return self.library.lib_tokens[self.tokens.idx[tuple(coords)]]

    def as_tokens(self):
        """
        Returns all phytokens contained in VectPrograms.
        Returns
        -------
        phytokens : numpy.array of shape (batch_size,max_time_step,) of phytokens.Token
        """
        return self.library.lib_tokens[self.tokens.idx]

    def get_prog_tokens(self, prog_idx=0):
        """
        Returns a list of phytokens representing a single program of idx = prog_idx.
        Discards void phytokens beyond program length.
        Parameters
        ----------
        prog_idx : int
        ...
        Returns
        -------
        phytokens : numpy.array of phytokens.Token
            Tokens making up program.
        """
        length = self.n_completed[prog_idx]
        # Keeping only phytokens before actual length of program
    ...
    def batch_exe_reward(self, X, y_target, X_names, reward_function, mask=None, pad_with=np.NaN, n_cpus=1, parallel_mode=False):
        """
    ...
    def batch_optimize_constants(self, X, y_target, free_const_opti_args=None, mask=None, n_cpus=1,
                                 parallel_mode=False):
        """
    ...
    def get_tree_graph(self, prog_idx=0,
        ...
        constraint_color : str
            Color for phytokens having constraining static physical units.
        vanilla_color : str
            Default color for phytokens.
        dummy_color : str
            Color for dummy placeholder phytokens.
        special_color : str
             Color for special phytokens.
        special_color_pos : int
            Position of special phytokens.
        edge_color : str
        ...
        def color_from_cursor(cursor):
            if cursor.pos == special_color_pos:
                color = special_color  # special color phytokens
            elif cursor.token_prop("idx") == self.library.dummy_idx:
                color = dummy_color  # dummy phytokens color
            elif cursor.token_prop("is_constraining_phy_units"):
                # is constraining due to position in program, not nature of phytokens
                color = constraint_color  # positional constraining units
            elif cursor.token.is_constraining_phy_units:
                color = constraint_color  # constraining units phytokens
            else:
                color = vanilla_color  # vanilla phytokens color
            return color

        # Utils function to add a node representing a phytokens
        ...
        # Utils function to add a node representing a phytokens from a Cursor object
        def node_from_cursor(cursor):
            units = cursor.token_prop("phy_units")
            if not cursor.token_prop("is_constraining_phy_units"):
                units = np.full(units.shape, '-')  # Fill with - instead of nan if there are no units in phytokens
            add_token_node(name=cursor.token.name,
                           pos=cursor.pos,
                           # Getting units constraints from VectPrograms (not static units constraints in phytokens)
                           units=units,
                           color=color_from_cursor(cursor), )
            return None

        # Initializing cursor pointing to first phytokens
