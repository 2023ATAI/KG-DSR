...
* :func:`Calc_Z_L` [Dyer1974]_ similarity function for momentum transfer.
...
def brutsaertstability(ZL):
  #  ZL     =  map(np.asarray, (ZL))
    y      =  ZL * -1
    y      =  np.where(y > 14.509,y,14.509)
    a      =  0.33
    b      =  0.41
    c      =  0.33
    d      =  0.057
    n      =  0.78
    S0     =  -np.log(a)+(3.0**0.5)*b*(a**0.3333)*np.pi/6.0
    x      =  (y / a)**(1.0/3.0)
    stab_m =  ZL
    stab_h =  ZL
    stab_m =  np.where( ZL >= 0.0, stab_m, -6.1*np.log(ZL+(1.+ZL**2.5)**(1.0/2.5)))
    stab_h =  np.where( ZL >= 0.0, stab_h, stab_m)
    stab_h =  np.where( ZL < 0.0, stab_h, ((1.0 - d) / n) * np.log(( c + y**n) / c) )
    stab_m =  np.where(y<=14.509, stab_m,
    np.log(a + y)-3.0*b*(y**(1.0 / 3.0 )) + b * (a**(1.0/3.0)) / 2.0 * 
    np.log(((1.0+x)**2.0) / (1.0-x+x*x))+(3.**0.5)*b*(a**(1.0/3.0))*np.arctan((2.0*x-1.0)/(3.0**0.5)) + S0)
    return stab_m,stab_h
...
def calc_mo_length_hv(ustar, T_A_K, rho, c_p, H, LE):
    ...
    Lambda =  2.501 - (2.361e-3* (T_A_K-273.15) ) #calc_lambda(T_A_K)  # in J kg-1
    E = LE / Lambda
    del LE, Lambda
...
def Calc_Z_L(Z,d0, L):
    Z,d0,L=map(np.asarray, (Z, d0,L))
    ZL=(Z-d0)/L
    return np.asarray(ZL)