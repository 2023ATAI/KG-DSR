...
def learner ( model,
    ...
    gamma_decay : float
        ...
        function (gamma_decay < 1 gives more important to first phytokens and gamma_decay > 1 gives more weight to last
        phytokens).
    ...
    for epoch in range (n_epochs):
        ...
        for i in range (max_time_step):
            ...
            # ------------ MODEL ------------
            prior_array = batch.prior().astype(np.float32)
            ...
            epsilon = 1e-14 #1e-14 #1e0*np.finfo(np.float32).eps
        ...
        if (stop_reward - overall_max_R_history[-1]) <= early_stop_reward_eps:
            if stop_after_n_epochs == 0:
                try:
                    ...
                    print("Unable to save last plots and data_utils before early stopping.")
                break
        ...
        if (max_n_evaluations is not None) and (n_evaluated + batch_size > max_n_evaluations):
            try:
                ...
                print("Unable to save last plots and data_utils before stopping due to max evaluation limit.")
            break
