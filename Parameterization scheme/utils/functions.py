...
from phytokens.Token import Token
...
class OpUnitsBehavior(object):
    ...
    op_names : list of str
        List of operation names having this behavior (eg. [sqrt, n2, n3] for power phytokens)
    behavior_id : int
...
class GroupUnitsBehavior(object):
    ...
    Eg: mul and div phytokens have their one unique behaviors regarding physical units (units of mul = units of arg1 +
    units of arg2 whereas units of div = units of arg1 - units of arg2) but they also share common behaviors that can
    be encoded here (in both cases it is impossible to guess the units of the phytokens unless both args' units  are known
    etc.).
    Attributes
    ----------
    ...
    op_names : list of str
        List of operation names having this behavior (eg. [mul, div,] for multiplicative phytokens)
...
# Unique (phytokens-wise) behaviors dict (ie. each phytokens must appear in only one of those behavior, if necessary use a
# group of unit behavior).
OP_UNIT_BEHAVIORS_DICT = {
    "DEFAULT_BEHAVIOR"          : OpUnitsBehavior(behavior_id = config.positional_token_default_values['DEFAULT_BEHAVIOR_ID'], op_names = []),
...
# Group of behaviors (phytokens can appear in more than one of them)
GROUP_UNIT_BEHAVIOR = {
...
# All behaviors (phytokens can appear in more than one of them)
UNIT_BEHAVIORS_DICT = {}
...
# Inverse of data_utils conversion
...
# iterating through all available phytokens
for token_op in OPS_PROTECTED + OPS_UNPROTECTED:
    # encoding phytokens behavior in dimensional analysis
    for _, behavior in OP_UNIT_BEHAVIORS_DICT.items():
        # Filtering out objects in the dict that are not meant to affect phytokens' behavior id
        if token_op.name in behavior.op_names:
            token_op.behavior_id = behavior.behavior_id
    # encoding dimensionless phytokens units
    if token_op.name in OP_UNIT_BEHAVIORS_DICT["UNARY_DIMENSIONLESS_OP"].op_names:
        token_op.is_constraining_phy_units = True
        token_op.phy_units                 = np.zeros((config.positional_token_default_values['UNITS_VECTOR_SIZE']))
    # encoding power phytokens values
    if token_op.name in OP_UNIT_BEHAVIORS_DICT["UNARY_POWER_OP"].op_names:
        ...
        except KeyError: raise UnknownFunction("Token %s is a power phytokens as it is listed in UNARY_POWER_OP "
...
def retrieve_complexity(complexity_dict, curr_name):
    """
    Helper function to safely retrieve complexity of phytokens named curr_name from a dictionary of complexities
    (complexity_dict).
    Parameters
    ----------
    complexity_dict : dict of {str : float} or None
        If dictionary is None, returns phytokens.DEFAULT_COMPLEXITY.
    curr_name : str
        If curr_name is not in units_dict keys, returns phytokens.DEFAULT_COMPLEXITY.
    Returns
    -------
    curr_complexity : float
        Complexity of phytokens.
    """
    curr_complexity = config.positional_token_default_values['DEFAULT_COMPLEXITY']
    if complexity_dict is not None:
        try:
            curr_complexity = complexity_dict[curr_name]
        except KeyError:
            warnings.warn(
                "Complexity of phytokens %s not found in complexity dictionary %s, using complexity = %f" %
...
def retrieve_init_val (init_val_dict, curr_name):
    """
    Helper function to safely retrieve value of phytokens named curr_name from a dictionary of initial values.
    (init_val_dict).
    Parameters
    ----------
    init_val_dict : dict of {str : float} or None
        If dictionary is None, returns phytokens.DEFAULT_FREE_CONST_INIT_VAL.
    curr_name : str
        If curr_name is not in units_dict keys, returns phytokens.DEFAULT_FREE_CONST_INIT_VAL.
    Returns
    -------
    curr_init_val : float
        Initial value of phytokens.
    """
    curr_init_val = config.positional_token_default_values['DEFAULT_FREE_CONST_INIT_VAL']
    if init_val_dict is not None:
        try:
            curr_init_val = init_val_dict[curr_name]
        except KeyError:
            warnings.warn(
                "Initial value of phytokens %s not found in initial value dictionary %s, using complexity = %f" %
...
def retrieve_units(units_dict, curr_name):
    """
    Helper function to safely retrieve units of phytokens named curr_name from a dictionary of units (units_dict).
    Parameters
    ----------
    units_dict : dict of {str : array_like} or None
        ...
        (Note: creating a phytokens.Token using None in place of units will result in a Token with units = vector of np.NAN)
    curr_name : str
        If curr_name is not in units_dict keys, returned curr_phy_units correspond to that of  dimensionless phytokens
        (ie. vector of zeros).
    Returns
    -------
    curr_is_constraining_phy_units, curr_phy_units : (bool, numpy.array)
        Does the phytokens require physical units, numpy array containing units
    """
    ...
    if units_dict is not None:
        ...
        try:
            ...
            curr_phy_units = np.zeros(config.positional_token_default_values['UNITS_VECTOR_SIZE'])
            warnings.warn(
                "Physical units of phytokens %s not found in units dictionary %s, assuming it is \
        ...
        assert curr_size <= config.positional_token_default_values['UNITS_VECTOR_SIZE'], 'Physical units vector has size = %i which exceeds max size = %i \
            (Lib.UNITS_VECTOR_SIZE, can be changed)' % (curr_size, config.positional_token_default_values['UNITS_VECTOR_SIZE'])
        curr_phy_units = np.pad(curr_phy_units, (0, config.positional_token_default_values['UNITS_VECTOR_SIZE'] - curr_size), 'constant')
...
def make_tokens(
    ...
    """
        Makes a list of phytokens for a run based on a list of operation names, input variables ids and constants values.
        Parameters
        ----------
        -------- operations --------
        op_names : list of str or str, optional
            ...
            to use all available phytokens. By default, op_names = "all".
        ...
        input_var_complexity : dict of { str : float } or None, optional
            ...
            (eg. 0., 1., 0.). If None, complexity = phytokens.DEFAULT_COMPLEXITY will be encoded to phytokens. None by default.
        -------- constants --------
        ...
        constants_complexity : dict of { str : float } or None, optional
            ...
            (eg. 0., 0., 1.). If None, complexity = phytokens.DEFAULT_COMPLEXITY will be encoded to phytokens. None by default.
        -------- free constants --------
        ...
        free_constants_init_val : dict of { str : float } or None, optional
            ...
            phytokens.DEFAULT_FREE_CONST_INIT_VAL as initial values. None by default.
        ...
        free_constants_complexity : dict of { str : float } or None, optional
            ...
            (eg. 1., 1., 1.). If None, complexity = phytokens.DEFAULT_COMPLEXITY will be encoded to phytokens. None by default.
        Returns
        -------
        list of phytokens.Token
            List of phytokens used for this run.
    ...
    # Using all available phytokens
    if op_names == "all":
        tokens_ops = list(ops_dict.values())
    # Appending desired functions phytokens
    else:
        # Iterating through desired functions names
        for name in op_names:
            # appending phytokens function if available
            try:
                tokens_ops.append(ops_dict[name])
            except KeyError:
                raise UnknownFunction("%s is unknown, define a custom phytokens function in functions.py or use a function \
