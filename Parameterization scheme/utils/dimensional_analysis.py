...
def assign_required_units_at_step (programs, step = None, from_scratch = False):
    """
    Usage: computes units that will be used to update units of dummy representing next phytokens to guess
    Must be able to work with coords instead of step to work so it can be used on next phytokens to guess dummies and not -
    void phytokens.
    Parameters
    ----------
    ...
    tokens_cases_record : numpy.array of shape (?,) of int
        Encountered physical units deduction case for each phytokens.
    """
    if step is None:
    ...
    assert step < programs.max_time_step, "Step argument = %i is out of range of programs of max_time_step = %i. " \
                                          "Next phytokens's units requirement can not be computed as there is no next " \
                                          "phytokens." % (step, programs.max_time_step)
    if from_scratch:
        for i in range (step):
...
def assign_required_units(programs, coords,):
    """
    Computes and assigns physical units requirements to phytokens at coords, works with complete or incomplete programs
    (containing dummies).
    In certain cases, a bottom-up physical units computation and assignment is performed on whole subtrees of
    programs as it is necessary to compute the units constraints of a particular phytokens.
    This function is able to provide the most constraining physical units constraints only if it was also ran at all
    positions before the phytokens at coords.

    This function fills is_constraining_phy_units a boolean property of phytokens indicating if the phytokens has a physical
    unit constraint associated to it and phy_units, a vector of floats containing the physical units associated with the
    phytokens. If it is impossible to compute the physical units constraints of a phytokens at the current step (which can
    happen in incomplete programs), is_constraining_phy_units is put to False and phy_units is filled with NaNs.

    This function handles all possible cases of phytokens arrangements situations relevant for live (working with
    unfinished trees) physical units constraints computation. Each case is associated with a unique id code, the id
    code of the cases in which phytokens at coords fell into are returned for analyses purposes.

    Using this function on phytokens of the same program simultaneously was not tested.
    ...
    coords : numpy.array of shape (2, ?) of int
        Coords of phytokens in programs for which physical units requirements should be computed and assigned, 0-th array
    ...
    assert (coords[1, :] < programs.max_time_step).all(), "Some phytokens have a position that is out of range of " \
                                                          "programs (max_time_step = %i)." % (programs.max_time_step)

    # -------------------- COORDS --------------------

    # mask : is phytokens in VectPrograms part of the selection considered here
    mask_tokens = programs.coords_to_mask(coords)                                                         # (batch_size, max_time_step,)
    # mask : is phytokens in VectPrograms already constraining ?
    ...
    # Array containing detected cases codes of each phytokens to process. By default = no_case_code
    ...
    # mask : do we have constraints regarding the physical units of the phytokens ?
    # By default = False (no constraint)
    is_constraining = np.full(shape = n_tokens, fill_value = False, dtype = bool)                         # (n_tokens,)

    # Required units for each phytokens in batch. By default = np.NAN (no constraint)
    phy_units = np.full(shape = (n_tokens, config.positional_token_default_values['UNITS_VECTOR_SIZE']), fill_value = np.NAN, dtype = float)   # (n_tokens, UNITS_VECTOR_SIZE)

    # -------------------- UTILS --------------------
    # Dummy phytokens's idx in the library
    dummy_idx = programs.library.dummy_idx
    # Current phytokens's idx in the library
    curr_token_idx                  = programs.tokens.idx  [tuple(coords)]                                 # (n_tokens,)
    # Current phytokens's arity
    curr_token_arity                = programs.tokens.arity[tuple(coords)]                                 # (n_tokens,)
    # Current phytokens is already constraining ?
    ...
    # mask : does phytokens have parent (ie. are any of the properties meaningful ie. they do not consist of filler values)
    ...
    # mask : does phytokens have sibling (ie. are any of the properties meaningful ie. they do not consist of filler values)
    ...
    # -------------------- ASSERTION --------------------
    # Can not work on void filler phytokens as they are not part of tree
    mask_invalid_tokens = (curr_token_idx == programs.invalid_idx)
    assert mask_invalid_tokens.sum() == 0, "Detected invalid void filler phytokens(s) at pos = %s, batch_pos = %s"%(
                                            pos, batch_pos)
    # Tokens at pos > 0 not having parents -> invalid trees
    mask_invalid_trees = (pos > 0) & (has_parent == False)
    assert mask_invalid_trees.sum() == 0, "Detected phytokens(s) at pos > 0 have no parents, programs at batch_pos %s " \
    ...
    # only process phytokens that were not processed before because phytokens may fall into several cases and the first case
    ...
    # -------------------- CASE 1 -------------------- #DONE
    # If the parent is an additive phytokens AND we know the sibling AND we know the sibling's physical units, the phytokens's
    # physical units must be that of the sibling regardless of the parent's units (even if it is free)
    case_code = 1

    # mask : does phytokens fall into this case ?
    ...
    # Number of phytokens falling into this case
    n_in_case = mask_case.sum()
    # Declaring phytokens that fell into this case in
    ...
    # -------------------- CASE 20 -------------------- #DONE
    # Elif the parent is an additive phytokens AND parent dim is free (eg: nested in left part of multiplicative phytokens) AND
    # it is the right child AND sibling dim is free then we can compute the sibling's physical units bottom up which
    # is the required units of the current phytokens at right part (right phytokens = 1th child of parent)
    case_code = 20
    ...
    # mask : phytokens for which we do not know the physical units of the sibling ie. for which we have to perform bottom
    ...
    # Start of subtree is the sibling of the current phytokens
    coords_start = programs.get_siblings(coords)       [:, mask_case_da_todo]
    # End of subtree is the phytokens just before the current phytokens (pos - 1)
    ...
    # Unless: 1. the parent is an additive phytokens, and we know everything about the sibling (already handled in case 1
    # above)
    # Unless: 2. the parent is an additive phytokens of free dim (eg. descendant of left part of multiplicative phytokens)
    # and we are dealing with right part, left part dim is complete and can thus be computed (already handled in case
    # 20 above)
    # Unless: 3. the parent is a multiplicative phytokens and the sibling is not a dummy. We use "& has_sibling" in
    ...
    # -------------------- CASE 3 -------------------- #DONE
    # Elif phytokens are at pos = 0 -> the required units are those of the superparent
    case_code = 3
    ...
    # -------------------- CASE 4 -------------------- #DONE
    # Elif parent is an additive phytokens (binary or unary), then its child must have same physical units
    case_code = 4
    ...
    # -------------------- CASE 5 -------------------- #DONE
    # Elif parent is a power phytokens. parent = (phytokens)**n_power => dim(parent) = n_power*dim(phytokens)
    case_code = 5
    ...
    tiled_power = np.tile(parent_power, reps=(
    config.positional_token_default_values['UNITS_VECTOR_SIZE'], 1)).transpose()                       # (n_tokens, UNITS_VECTOR_SIZE)
    # Handling is_constraining and phy_units
    apply_constraints(mask_case             = mask_case,
                      const_is_constraining = True,
    ...
    # -------------------- CASE 6 -------------------- #DONE
    # Elif parent is a dimensionless phytokens.
    case_code = 6
    ...
    # -------------------- CASE 7 -------------------- #DONE
    # Parent is a multiplicative phytokens
    ...
    # notation) of a binary multiplicative phytokens) then we can not determine the dim (we must leave it as a free
    # parameter). This also applies to when we are in the left or the right part of a subtree but both phytokens are
    ...
    # -------------------- CASE 71 -------------------- #DONE
    # Elif we are in left part of subtree then sibling is unknown therefore units of current phytokens is unknown.
    ...
    # free phytokens. Useful when doing units determination from_scratch.
    case_code = 71
    ...
    # -------------------- CASE 72 -------------------- #DONE
    # Elif we don't have units constraints on the parent (ie. if there are nested multiplicative phytokens and we are in
    ...
    # -------------------- CASE 73 -------------------- #DONE
    # Else: this should be applied to ALL phytokens left in general_case mask (parent is a multiplicative phytokens) that were
    # not already handled in the above subcases (70, 71, 72). If we already have access to the sibling (when dealing
    # with the right part of the tree ie 2nd arg of a binary multiplicative phytokens) then we can deduce the physical
    ...
    # Handling multiplication phytokens parents
    # parent = sibling x phytokens => dim(phytokens) = dim(parent) - dim(sibling)
    ...
    # Handling division phytokens parents
    # parent = sibling/phytokens => dim(phytokens) = dim(sibling) - dim(parent)
    ...
    # -------------------- CASE : NO CASE --------------------
    # Check that all phytokens were handled (ie. that they all fell into one of the above cases)
    # Even if it is sometimes impossible to compute required units, all cases (including those) should be covered by
    # this function, if a phytokens does not fall into any of the above cases, raise an error
    # mask : does phytokens not fall into any of the above cases ?
    mask_no_case = (tokens_cases_record == no_case_code)                                                  # (n_tokens,)
    n_no_case = mask_no_case.sum()
    assert n_no_case == 0, "Failure to process units requirements of %i phytokens. Positions in batch = %s ; positions in" \
                            " sequence = %s (no corresponding case found)." % (
                            n_no_case, batch_pos[mask_no_case], pos[mask_no_case])

    # -------------------- ASSIGNMENT --------------------
    # Only assign units to phytokens for which we don't already have units information
    ...
    # case they are replaced by terminal phytokens but this can cause problem when computing all constraints from scratch.)
    ...
    # print("is_constraining_phy_units", programs.phytokens.is_constraining_phy_units[mask_tokens][0])
    # print("phy_units", programs.phytokens.phy_units[mask_tokens][0])
    # print("tokens_cases_record", tokens_cases_record)

    # -------------------- REGISTERING UNITS ANALYSIS RUN --------------------
    # Registering that required units assignment was performed on phytokens at coords with the encountered codes.
...
def assign_units_bottom_up (programs, coords_start, coords_end):
    ...
    for k in range (n_subtrees):
        ...
        prog_i = batch_pos [k]
        # Error messages
        error_msg_unknown_dim          = "Unknown physical units phytokens encountered in bottom up units assignment " \
                                         "process."
        error_msg_dimensionless_child  = "Non-dimensionless phytokens encountered as child of dimensionless op (eg cos, " \
                                         "exp, log etc) in bottom up units assignment process."
        error_msg_dimensionless_token  = "Dimensionless phytokens having non-zero physical units encountered in bottom " \
        ...
        error_msg_incomplete_tree      = "Regular bottom up dimensional analysis can not be performed on incomplete " \
                                         "tree (containing terminal phytokens with unknown physical units: eg. dummies)"
        ...
        def parser(i):
            ...
            # Arity = 1 ---
                ...
                # Position of the lonely child of the phytokens (arity = 1)
                ...
                # Making sure that the child of unary phytokens are not free
                assert child_is_constraining == True, error_msg_unknown_dim
                # If phytokens is an unary power op -> apply power to units
                ...
                # Elif phytokens is an unary additive op -> copy-paste units from child
                ...
                # Elif phytokens is an unary dimensionless op -> nothing to do but making sure that child phytokens is
                # dimensionless (as it should be) just in case and that current phytokens is dimensionless
                elif Func.UNIT_BEHAVIORS_DICT["UNARY_DIMENSIONLESS_OP"].is_id(behavior_id):
                    assert np.array_equal(child_phy_units, np.zeros(
                        config.positional_token_default_values['UNITS_VECTOR_SIZE'])) and \
                           child_is_constraining == True, error_msg_dimensionless_child
                    assert np.array_equal(phy_units, np.zeros(
                        config.positional_token_default_values['UNITS_VECTOR_SIZE'])) and \
            ...
            # Arity = 2 ---
                ...
                # Assertion: making sure that children of binary phytokens are not free
                assert (child0_is_constraining == True) and (child1_is_constraining == True), error_msg_unknown_dim
                # If phytokens is an additive phytokens -> units are those of any children (as they should be the same
                # among them) but making sure that children of additive binary phytokens have the same units for safety.
                ...
                # Elif phytokens is a multiplicative phytokens
                elif Func.UNIT_BEHAVIORS_DICT["BINARY_MULTIPLICATIVE_OP"].is_id(behavior_id):
                    # phytokens = child0 * child1 => units(phytokens) = child0_phy_units + child1_phy_units
            ...
            # print("------")
            # print("Parsing i = ",i)
            # print("name      = ", programs.lib_names[programs.phytokens.idx[prog_i, i]])
            # print("units     = ", programs.phytokens.phy_units[prog_i, i])
            # Parsing previous phytokens afterwards
            parser(i - 1)
        parser(start)
