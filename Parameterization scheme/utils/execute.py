...
def ExecuteProgram (input_var_data, program_tokens, free_const_values=None):
    ...
    # Number of phytokens in the program
    n_tokens = len(program_tokens)

    # Current stack of computed results
    curr_stack = []

    # De-stacking program (iterating from last phytokens to first)
    start = n_tokens - 1
    for i in range (start, -1, -1):
        token = program_tokens[i]
        # Terminal phytokens
        if token.arity == 0:
            # Function type phytokens
            if token.var_type == 0:
                #curr_stack.append(phytokens.function())
            ...
            # Free constant variable (eg. c0, c1 etc.)
                ...
                if free_const_values is not None:
                    # curr_stack.append(torch.abs(free_const_values[phytokens.var_id])) # Making free const positive values only #abs_free_const
...
def ComputeInfixNotation (program_tokens):
    ...
    ----------
    program_tokens : list of token.Token
        List of phytokens making up the program.
    Returns
    -------
    program_str : str
    """
...
def BatchExecution (progs, X, mask = None, n_cpus = 1, parallel_mode = False):
    ...
    # Number of data_utils point per dimension
    n_samples = X.shape[1]
...
# Utils pickable function (non nested definition) executing a program (for parallelization purposes)
def task_exe_reward(prog, X, y_target, X_names, reward_function):
    task_name = 'GS'
    y_pred_ = prog(X)
    if task_name =='GS':
        var_indices = {var: i for i, var in enumerate(X_names)}
        Rnet = X[var_indices["Rn"]]
        Qg = X[var_indices["G"]]
        delta = X[var_indices["delta"]]
        rho = X[var_indices["rho"]]
        Cp = X[var_indices["Cp"]]
        VPD = X[var_indices["VPD"]]
        Ga = X[var_indices["Ga"]]
        Psy = X[var_indices["Psy"]]
        LAI = X[var_indices["LAI"]]
        y_pred = (delta * (Rnet - Qg) + rho * Cp * VPD * Ga) / (delta + Psy * (1 + Ga / (1./(72/(LAI*y_pred_)))))
    else:
        y_pred = y_pred_
...
def BatchExecutionReward (progs, X, y_target, X_names, reward_function, mask = None, pad_with = np.NaN, n_cpus = 1, parallel_mode = False):
    """
    ...
    # ----- Non parallel mode -----
        ...
        for i in pb(range(progs.batch_size)):
            ...
            if mask[i]:
                prog = progs.get_prog(i, skeleton=True)
                result = task_exe_reward(prog, X, y_target, X_names, reward_function)              # float
                results.append(result)
