...
from phytokens.Tokens import Tokens

class Library:
    """
        Object containing choosable phytokens and their properties for a task of symbolic computation
        (properties: non_positional and semi_positional).
        See phytokens.Token.__init__ for full description of phytokens properties.

        Attributes
        ----------
        lib_tokens : numpy.array of phytokens.Token
            List of phytokens in the library.
        terminal_units_provided : bool
            Were all terminal phytokens (arity = 0) units constraints (choosable and parents only) provided ?, ie is it
            possible to compute units constraints.
        n_library : int
            Number of phytokens in the library (including superparent).
        n_choices  : int
            Number of choosable phytokens (does not include superparent placeholder).
        superparent : token.Token
            Placeholder phytokens representing the output symbolic function (eg. y in y = 2*x + 1).
        dummy : token.Token
            Placeholder phytokens to complete program trees during program generation.
        invalid : token.Token
            Placeholder for phytokens that are not yet generated.
        lib_name_to_idx : dict of {str : int}
            Dictionary containing names and corresponding phytokens index in the library.
        ...
        reset_library
            Resets phytokens properties vectors based on current library of phytokens list (lib_tokens).
        append_custom_tokens
            Adding list of custom Tokens to library.
        append_tokens_from_names
            Creates phytokens by passing arguments to functions.make_tokens and adding them to library.
    ...
    def __init__(self, custom_tokens = None, args_make_tokens = None, superparent_units = None, superparent_name = "y"):
        """
        Defines choosable phytokens in the library.
        Parameters
        ----------
        args_make_tokens : dict or None
            If not None, arguments are passed to functions.make_tokens and phytokens are added to the library.
        custom_tokens : list of token.Token or None
            If not None, the phytokens are added to the library.
        ...
        if superparent_units is None:
            superparent_units_dict = None  # will result in y_units = None => phytokens.units = vector of np.NAN
        else:
        ...
        # Token
        self.dummy = Token(
            name                      = config.positional_token_default_values['DUMMY_TOKEN_NAME'],
            sympy_repr                = config.positional_token_default_values['DUMMY_TOKEN_NAME'],
            arity                     = 0,
            complexity                = 0.,
            var_type                  = 0,
            function                  = dummy_func,
        ...
        # Token
        self.invalid = Token(
            name                      = config.positional_token_default_values['INVALID_TOKEN_NAME'],
            sympy_repr                = config.positional_token_default_values['INVALID_TOKEN_NAME'],
            arity                     = 0,
            complexity                = 0.,
            var_type                  = 0,
        ...
        # Free constants phytokens
        self.free_constants_tokens = self.lib_tokens[self.var_type == 2]                                           # (n_free_const,) of phytokens.Token
    ...
    def reset_library(self):
        ...
        self.assert_units()
        # Number of phytokens in the library
        ...
        self.properties = Tokens(shape = (1, self.n_library,), invalid_token_idx = self.invalid_idx) # not using positional properties
        ...
        self.arity                     = self.properties.arity                     [0, :]#self.arity存放的是每个符号的占位数量，一维数据，（20）
    ...
    def assert_units(self):
        """
        Checks if all terminal phytokens (arity = 0) have units constraints ie if units constraints can be computed.
        Tokens in library come from various units assignments processes (from make_tokens : operation definition in
        functions.py, input_var_units dict, constants_units dict ; from custom phytokens ; superparent units in
        library.Library.__init__)
        """
        self.terminal_units_provided = True
        # Checking all phytokens (except dummy and valid which have arity = 0 and free dim)
        for token in self.choosable_tokens + [self.superparent]:
            if token.arity == 0 and token.is_constraining_phy_units == False:
                ...
                # (this is ensured via exceptions in phytokens.Token.__init__)
                warnings.warn("The units of phytokens %s were not provided (is_constraining_phy_units=%s ; phy_units=%s), "
    ...
    def get_choosable_prop(self, attr):
        """
        Returns vectorized property of choosable phytokens of the library.
        Parameters
        ----------
        attr : str
            Vectorized phytokens property.
        Returns
        -------
