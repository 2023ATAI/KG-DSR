...
from configs.config import config0
from reward import reward
# Internal imports
from configs import config
import learn.monitoring as monitoring
from sym_r.fit import fit
...
default_op_names = config.default_op_names
default_stop_after_n_epochs = config.default_stop_after_n_epochs
...
def SR(X, y,
    ...
    op_names : array_like of shape (?) of str or None (optional)
        ...
        By default, uses operations listed in physo.task.sym_r.default_op_names.
    ...
    run_config : dict (optional)
        Run configuration (by default uses physo.task.sym_r.default_config)
    get_run_logger : callable returning physo.learn.monitoring.RunLogger (optional)
        Run logger (by default uses physo.task.sym_r.get_default_run_logger)
    get_run_visualiser : callable returning physo.learn.monitoring.RunVisualiser (optional)
        Run visualiser (by default uses physo.task.sym_r.get_default_run_visualiser)
    ...
    run_config["learning_config"]["rewards_computer"] = reward.make_RewardsComputer(**reward_config)
    ...
    rewards, candidates = fit (X, y, X_names, run_config,
