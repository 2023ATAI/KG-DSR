...
# Internal imports
import phytokens
from program.VectPrograms import VectPrograms
from library import library
from prior.PriorCollection import make_PriorCollection
from sym_r import dataset
...
class Batch:
    ...
    Input  :
        ----- per step -----
        - new phytokens can be appended by their int idx in the library of choosable phytokens.
    Output :
        ----- per step -----
        - prior values for choice of next phytokens.
        - environment of next phytokens to guess parent/sibling one hots etc.
        ----- per epoch -----
    ...
    def __init__(self,
        ...
        # Programs
        self.programs = VectPrograms(batch_size        = self.batch_size,
                                             max_time_step     = self.max_time_step,
                                             library           = self.library,
                                             candidate_wrapper = candidate_wrapper)
        # Prior
        self.prior   = make_PriorCollection(programs      = self.programs,
                                                  library       = self.library,
    ...
    def get_sibling_one_hot (self, step = None):
        """
        Get siblings one hot of phytokens at step. 0 one hot vectors for dummies.
        Parameters
        ----------
        step : int
            Step of phytokens from which sibling one hot should be returned.
        ...
        # Do phytokens have siblings : mask
        has_siblings_mask = np.logical_and(                                         # (batch_size,)
            self.programs.tokens.has_siblings_mask[:, step],
            siblings_idx < self.programs.library.n_choices) # gets rid of dummies phytokens which are valid siblings
    ...
    def get_parent_one_hot (self, step = None):
        """
        Get parents one hot of phytokens at step.
        Parameters
        ----------
        step : int
            Step of phytokens from which parent one hot should be returned.
            By default, step = current step
        Returns
        -------
        one_hot : numpy.array of shape (batch_size, n_choices) of int
        ...
        # Do phytokens have parents : mask
    ...
    def get_previous_tokens_one_hot(self):
        """
        Get previous step phytokens as one hot.
        Returns
        -------
        one_hot : numpy.array of shape (batch_size, n_choices) of int
            One hot.
        ...
        if self.programs.curr_step == 0:
            ...
            # Idx of phytokens at previous step
            tokens_idx = self.programs.tokens.idx[:, self.programs.curr_step - 1]  # (batch_size,)
            # Are these phytokens outside of library (void phytokens)
            valid_mask = self.programs.tokens.idx[:,
                         self.programs.curr_step - 1] < self.library.n_choices     # (batch_size,)
            # Initialize one hot result
            one_hot = np.zeros((self.batch_size, self.library.n_choices))          # (batch_size, n_choices)
            # Affecting only valid phytokens and leaving zero vectors where previous vector has no meaning
    ...
    def get_sibling_units_obs (self, step = None):
        """
        Get (required) units of sibling of phytokens at step. Filling using INTERFACE_UNITS_UNAVAILABLE_FILLER where units
        ...
        ----------
        step : int
            Step of phytokens which's sibling's (required) units be returned.
            By default, step = current step.
        Returns
        -------
        units_obs : numpy.array of shape (batch_size, phytokens.UNITS_VECTOR_SIZE + 1) of float
        ...
        units_obs = np.zeros((self.batch_size,configs.config.positional_token_default_values['UNITS_VECTOR_SIZE'] + 1), dtype=float)              # (batch_size, UNITS_VECTOR_SIZE + 1)
        # filling units
        units_obs[:, :-1] = INTERFACE_UNITS_UNAVAILABLE_FILLER(                                         # (batch_size, UNITS_VECTOR_SIZE)
            shape=(self.batch_size, configs.config.positional_token_default_values['UNITS_VECTOR_SIZE']))
        # availability mask
    ...
    def get_parent_units_obs (self, step = None):
        """
        Get (required) units of parent of phytokens at step. Filling using INTERFACE_UNITS_UNAVAILABLE_FILLER where units
        are not available. Adding a vector in addition to the units indicating if units are available or not (equal to
        INTERFACE_UNITS_AVAILABLE where units are available and equal to INTERFACE_UNITS_UNAVAILABLE where there are no
        units infos available).
        Parameters
        ----------
        step : int
            Step of phytokens which's parent's (required) units be returned.
            By default, step = current step.
        Returns
        -------
        units_obs : numpy.array of shape (batch_size, phytokens.UNITS_VECTOR_SIZE + 1) of float
    ...
    def get_previous_tokens_units_obs (self, step = None):
        """
        Get (required) units of phytokens before step. Filling using INTERFACE_UNITS_UNAVAILABLE_FILLER where units are not
        ...
        ----------
        step : int
            Step of phytokens which's previous phytokens' (required) units be returned.
            By default, step = current step.
        Returns
        -------
        units_obs : numpy.array of shape (batch_size, phytokens.UNITS_VECTOR_SIZE + 1) of float
    ...
    def get_tokens_units_obs (self, step = None):
        """
        Get (required) units of phytokens at step. Filling using INTERFACE_UNITS_UNAVAILABLE_FILLER where units are not
        available. Adding a vector in addition to the units indicating if units are available or not (equal to
        INTERFACE_UNITS_AVAILABLE where units are available and equal to INTERFACE_UNITS_UNAVAILABLE where there are no
        units infos available).
        Parameters
        ----------
        step : int
            Step of phytokens which's (required) units be returned.
            By default, step = current step.
        Returns
        -------
        units_obs : numpy.array of shape (batch_size, phytokens.UNITS_VECTOR_SIZE + 1) of float
        ...
        # Coords of phytokens which's units are available
        ...
        units_obs[~is_available, :-1] = INTERFACE_UNITS_UNAVAILABLE_FILLER(                             # (n_unavailable, UNITS_VECTOR_SIZE)
            shape=(n_unavailable, configs.config.positional_token_default_values['UNITS_VECTOR_SIZE']))
    ...
    def obs_size(self):
        ...
        obs_size : int
        """
        return (3*self.n_choices) + 1 + 4*(configs.config.positional_token_default_values['UNITS_VECTOR_SIZE'] + 1)
